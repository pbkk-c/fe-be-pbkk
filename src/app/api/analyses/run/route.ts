...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ce || !content)
      return NextResponse.json({ error: "Content not found or has no raw_text" }, { status: 404 });
    ...
    let summary = null;
    try {
      const sumResp = await hfInference("facebook/bart-large-cnn", {
        inputs: rawText,
      ...
      if (Array.isArray(sumResp)) {
        summary =
          (sumResp[0] && (sumResp[0].summary_text || sumResp[0].generated_text)) ??
          String(sumResp[0]);
    ...
    let sentiment = null;
    try {
      const sentResp = await hfInference("distilbert-base-uncased-finetuned-sst-2-english", {
        inputs: rawText,
      });
    ...
    for (const s of sentences) {
      try {
        ...
        const label: string | undefined = zeroResp.labels?.[0] ?? zeroResp[0]?.label ?? undefined;

        const score: number | null = zeroResp.scores?.[0] ?? zeroResp[0]?.score ?? null;
